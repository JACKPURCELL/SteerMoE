...
class MLPBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        TEMP_NPY_PATH = f"{TEMP_NPY_BASE_PATH}/router_logits_L{self.layer_idx_}.npy"
        router_logits_np = g.cpu().float().numpy()
...
class GptOssForCausalLM(nn.Module):
    ...
    ### ADDED BY MOHSEN {
    def update_moe_manual_args(self):
        for layer_idx, layer in enumerate(self.model.layers):
            print(f"Shared layer idx in moe block: layer_idx={layer_idx}")
