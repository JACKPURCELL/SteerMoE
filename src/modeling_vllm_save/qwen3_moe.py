...
class Qwen3MoeForCausalLM(nn.Module, SupportsPP):
    ...
    def update_moe_manual_args(self):
        for layer_idx, layer in enumerate(self.model.layers):
            ...
            # Only set layer_idx for MoE layers (Qwen3MoeSparseMoeBlock)
            if isinstance(layer_moe_block, Qwen3MoeSparseMoeBlock):
                layer_moe_block.layer_idx = layer_idx
                print(f"update: MoE layer_idx={layer_idx}")
            else:
                print(f"update: layer_idx={layer_idx} is NOT a MoE layer (using standard MLP)")
    ### ADDED BY MOHSEN }
