...
class OLMOEFWOHandler(BaseDatasetHandler):
    """Handler for OLMOE-FWO jailbreak attack evaluation dataset."""
    
    def __init__(self, dataset_path: Optional[str] = None):
        """
        Initialize OLMOE-FWO handler with custom dataset path.
        
        Args:
            dataset_path: Path to the OLMOE-FWO.json file.
                         Defaults to /home/stufs1/jiachliang/SteerMoE/OLMOE-FWO.json
        """
    ...
    def load_dataset(self) -> Dataset:
        """Load OLMOE-FWO dataset from JSON file."""
        ...
        for item in dataset:
            ...
            if item.get("judge_success_gpt4", 0) == 0:
                continue
    ...
    def prepare_prompt(self, sample: Dict[str, Any]) -> Any:
        """
        Prepare prompt for OLMOE-FWO evaluation.
    ...
    def calculate_metrics(self, results: list) -> Dict[str, float]:
        ...
        print(f"OLMOE-FWO Safety Rate: {safety_rate:.4f} ({safe_count}/{len(results)} safe responses)")
